...
from precisionrx_agent.layer4_estimation.basis import BLIP_BASIS, blip_basis
...
class ModelExplainer:
    ...
    def _attributions(self, selected: MethodResult, stages: list[StageRecord]) -> list[BlipAttribution]:
        ...
        prefix = f"psi:{selected.recommended_action}:"
    ...
    def _why_not(self, selected: MethodResult) -> list[WhyNotEntry]:
        best = selected.q_values[selected.recommended_action]
        entries: list[WhyNotEntry] = []
        for action, value in sorted(selected.q_values.items(), key=lambda kv: kv[1], reverse=True):
            if action == selected.recommended_action:
                continue
            entries.append(
                WhyNotEntry(
                    action=action,
    ...
    def _counterfactuals(self, selected: MethodResult, stages: list[StageRecord]) -> list[CounterfactualProbe]:
        latest = stages[-1]
    ...
    def _sensitivity(self, selected: MethodResult) -> AssumptionSensitivity:
        ordered = sorted(selected.q_values.values(), reverse=True)
