...
from precisionrx_agent.layer4_estimation import linalg
from precisionrx_agent.layer4_estimation.basis import (
    BLIP_BASIS,
    TREATMENT_FREE_BASIS,
    blip_basis,
    treatment_free_basis,
)
from precisionrx_agent.layer4_estimation.features import model_features
from precisionrx_agent.layer4_estimation.q_learning import Q_CEILING, Q_FLOOR
...
class ArmFit:
    ...
    def __init__(self, arm: str, rows: list[tuple[dict[str, float], float, float]]) -> None:
        self.arm = arm
        self.n = len(rows)
...
class DWOLSModel:
    ...
    def __init__(
        ...
        observations = [
            (dict(stage.features), stage.arm, stage.outcome)
        ...
        for arm in arms:
            if arm == reference:
                continue
            rows = [
                (features, 1.0 if observed == arm else 0.0, outcome)
                for features, observed, outcome in observations
                if observed in (arm, reference)
            ]
            self.fits[arm] = ArmFit(arm, rows)
    ...
    def _fit_reference(self, observations) -> list[float]:
        ...
        rows = [(features, outcome) for features, arm, outcome in observations if arm == self.reference]
...
def fitted_model() -> DWOLSModel:
    ...
    global _MODEL
    if _MODEL is None:
        from precisionrx_agent.layer4_estimation.training import training_cohort
...
class DWOLSSharedEstimator:
    ...
    def fit_predict(
        ...
        from precisionrx_agent.layer4_estimation.training import policy_value_for
        ...
        half_width = self._half_width(model, recommended)
        return MethodResult(
    ...
    def _half_width(self, model: DWOLSModel, arm: str) -> float:
        fit = model.fits.get(arm)
        support = fit.n if fit else model.n_train
        return round(clamp(1.2 / max(support, 1) ** 0.5, 0.04, 0.25), 3)
